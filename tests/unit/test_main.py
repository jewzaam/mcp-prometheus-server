...
def test_main_with_duplicate_arguments():
    ...
    result = main([
        "--log-level", "DEBUG",
        "--log-level", "INFO"  # Second one should override
    ])
    assert result == 0
...
def test_main_with_special_shell_characters():
    ...
    result = main([
        "--prometheus-url", "https://example.com; rm -rf /",
        "--auth-token", "token && echo 'hacked'"
    ])
    assert result == 0
...
def test_main_with_unicode_arguments():
    ...
    result = main([
        "--prometheus-url", "https://服务器.example.com",
        "--auth-token", "токен-тест"
    ])
    assert result == 0
